...
private:
    ...
    // times are in global millis
    uint64_t last_active_error_times_millis[identifiers_count]; // last time the error was active   (TODO: possibly make this a 2d array to hold multiple times per id)
...
public:
    ...
    uint64_t get_last_reset_time_millis(void);  // get the last time all logs were reset
    uint64_t get_last_active_error_time_millis(uint32_t  id);  // get the last time the id was active
