...
if __name__ == "__main__":
    ...
    servos.axis0.controller.config.control_mode=3   # pos mode
    servos.axis0.controller.input_vel=0
    servos.axis0.controller.input_torque=0
    servos.axis0.requested_state=8
    x=0
    while(servos.axis0.current_state != 8):
        time.sleep(.1)
        x+=1
        if(x>=5):
            raise ("Could not enable motors(Odrive fail)")
    ...
    while True:
        ...
        if time.time() > lastUpdateTime + (1/updateFrequency):  # send data to nucelo
            ...
            servos.axis0.controller.input_pos = app.angle_var.get() / 360
            servos.axis0.controller.config.pos_gain = app.p_gain_var.get() * int(app.p_gain_enabled)
            servos.axis0.controller.config.vel_gain = app.d_gain_var.get() * int(app.d_gain_enabled)
            servos.axis0.controller.config.vel_integrator_gain = app.i_gain_var.get() * int(app.i_gain_enabled)
